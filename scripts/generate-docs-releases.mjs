...
const outputMarkdownPath = resolve(repoRoot, 'apps/docs/src/content/docs/releases/index.md');
...
const tagRowsRaw = run("git for-each-ref --merged=HEAD --sort=-v:refname --format='%(refname:short)|%(creatordate:short)|%(subject)' refs/tags");
...
const rows = tagRowsRaw
  ...
  // The release script generates this page before creating the current tag.
  // Render that version separately so tagging cannot change committed docs.
  .filter((row) => row.tag.replace(/^v/, '') !== currentVersion)
  .slice(0, 100);

const remote = run('git remote get-url origin');
const repoUrl = toGitHubHttpUrl(remote);
const currentRelease = repoUrl
  ? `[**${currentVersion}**](${repoUrl}/releases/tag/v${encodeURIComponent(currentVersion)})`
  : `**${currentVersion}**`;
...
Current package version from \`packages/scholar-mcp/package.json\`: ${currentRelease}.
