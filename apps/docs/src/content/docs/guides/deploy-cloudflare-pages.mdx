...
- `cf:pages:deploy`: `wrangler pages deploy ./dist --project-name scholar-mcp-docs`
- `cf:pages:deploy:preview`: `wrangler pages deploy ./dist --project-name scholar-mcp-docs --branch ${WORKERS_CI_BRANCH:-preview}`
- `cf:pages:deploy:ci`: `env -u CLOUDFLARE_API_TOKEN wrangler pages deploy ./dist --project-name scholar-mcp-docs`
- `cf:pages:deploy:preview:ci`: `env -u CLOUDFLARE_API_TOKEN wrangler pages deploy ./dist --project-name scholar-mcp-docs --branch ${WORKERS_CI_BRANCH:-preview}`
...
- This repo's `:ci` deploy scripts explicitly unset `CLOUDFLARE_API_TOKEN` to avoid accidental use of a mis-scoped token.
